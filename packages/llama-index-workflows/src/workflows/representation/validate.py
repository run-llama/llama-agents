...
if TYPE_CHECKING:
    # Workflow imports this module inside validation to break the import cycle.
...
def _validate_child_workflow_declarations(
    ...
    """
    _validate_child_type_graph(type(workflow))
    _validate_child_start_events(workflow)


def _validate_child_type_graph(root: type[Workflow]) -> None:
    """Reject cycles in the declared child-workflow type graph."""

    def walk(
        node: type[Workflow],
        path: tuple[type[Workflow], ...],
    ) -> None:
        for child_type in node._get_child_workflow_slots().values():
            if child_type in path:
                chain = " -> ".join(c.__name__ for c in (*path, child_type))
                raise WorkflowValidationError(
                    f"Child workflow type cycle detected: {chain}. A child "
                    "workflow cannot transitively declare itself as a child."
                )
            walk(child_type, (*path, child_type))

    walk(root, (root,))


def _validate_child_start_events(workflow: Workflow) -> None:
    """Direct child start types must not overlap by inheritance."""
    seen: dict[type[StartEvent], str] = {}
    for field_name, child in workflow._child_workflows.items():
        start_cls = child._start_event_class
        for other_cls, other_field in seen.items():
            if issubclass(start_cls, other_cls) or issubclass(other_cls, start_cls):
                overlap = (
                    f"both accept StartEvent type '{start_cls.__name__}'"
                    if start_cls is other_cls
                    else f"have overlapping StartEvent types "
                    f"'{other_cls.__name__}' and '{start_cls.__name__}'"
                )
                raise WorkflowValidationError(
                    f"Child workflows '{other_field}' and '{field_name}' on "
                    f"'{type(workflow).__name__}' {overlap}; "
                    "each child start must route to exactly one child."
                )
