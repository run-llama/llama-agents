...
@pytest.mark.xfail(
    reason="Collect(at=) cross-level scope promotion not implemented yet (L3 follow-up)",
    raises=WorkflowValidationError,
    strict=True,
)
async def test_two_level_at_outer_join() -> None:
    class Inner(Event):
        outer: int
    ...
    class FanOut(Workflow):
        ...
        @step
        async def inner(self, ev: Task) -> list[Inner]:
            return [Inner(outer=ev.n) for _ in range(2)]

        @step
        async def work(self, ev: Inner) -> InnerDone:
            return InnerDone(outer=ev.outer)

        @step
        async def per_outer(
            self, events: Annotated[list[InnerDone], Collect(at="outer")]
        ) -> StopEvent:
            return StopEvent(result=len(events))
...
@pytest.mark.xfail(
    reason="Multi-slot (two list[E] params) joins not implemented yet (L3 follow-up)",
    raises=WorkflowValidationError,
    strict=True,
)
async def test_two_converging_fan_outs_merge() -> None:
    class QueryDone(Event):
        n: int

    class FetchDone(Event):
        n: int
    ...
    class FanOut(Workflow):
        ...
        @step
        async def query(self, ev: Task) -> QueryDone:
            return QueryDone(n=ev.n)

        @step
        async def merge(
            self, queries: list[QueryDone], fetches: list[FetchDone]
        ) -> StopEvent:
            return StopEvent(result=len(queries) + len(fetches))
