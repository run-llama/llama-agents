...
class InheritedSlotWithUnresolvableAnnotation(Parent):
    helper: NeverImportedAtRuntime
""",
    globals(),
)
...
def test_inherited_child_slot_with_unresolvable_annotation() -> None:
    parent_cls = globals()["InheritedSlotWithUnresolvableAnnotation"]
    parent = parent_cls(child=Child(), timeout=30)
    assert parent.child_workflows == {"child": parent.child}
    assert parent._timeout == 30
